package com.leafup.leafupbackend.image.api.docs;
...
public interface ImageControllerDocs {
    ...
    @Operation(summary = "챌린지 이미지 업로드", description = "챌린지 이미지 업로드 합니다")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "업로드 성공"),
            @ApiResponse(responseCode = "401", description = "인증실패", content = @Content(schema = @Schema(example = "INVALID_HEADER or INVALID_TOKEN"))),
    })
    ResponseEntity<RspTemplate<String>> imageChallengeUpload(@AuthenticatedEmail String email,
